...
createSudokuFromFilled (n':ns) board@(board', size@(m, n)) 
  ...
  where
    board'' = (chunksOf (m * n) . removeith n' . zip [1..] . concat $ board', size)
    removeith :: Int -> [(Int, Cell)] -> [Cell]
    removeith n ((i, c) : cs)
      | i == n    = Choices (Set.fromAscList [1..(m*n)]) : map snd cs
      | otherwise = c : removeith n cs
...
generateBoardAsString m n = do
  seedRow          <- getSeedRow (m * n)
  toRemove         <- getSeedRow ((m * n) * (m * n))
  let validBoard   = createFilledBoard m n seedRow
  let prunedBoard  = createSudokuFromFilled toRemove validBoard
  return . showBoard $ prunedBoard
