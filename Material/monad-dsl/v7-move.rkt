...
;; Move `do-it` to the "monad.rkt" library. A monad implementation
;; is declared and packaged with `define-monad` so `do-it` can
;; refer to the right `return` and `bind`. The `return` form is
;; also provided by "monad.rkt" to constrain the use of `return`
;; to a monad context.
...
;; A `List` instance is a Racket list
