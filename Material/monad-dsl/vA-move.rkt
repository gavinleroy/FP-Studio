...
;; Suppose that we're only interested in find the best move,
;; so we can cut off the search when we find a winner. Do that
;; by changing the monad representation to have a `winner?`
;; flag --- very much like have a `winner?` mutable state variable!
...
(struct state (boards winner?))

;; A `List` instance is a (state -> state)

(define (extract-list m)
  (reverse (state-boards (m (state empty #f)))))

(define-monad List
  #:return (lambda (v)
             (lambda (accum)
               (struct-copy state accum
                            [boards (cons v (state-boards accum))])))
  #:bind (lambda (v k)
           (lambda (accum)
             (cond
               [(eq? v 'set-winner)
                ((k (void))
                 (if (state-winner? accum)
                     accum
                     (state empty #t)))]
               [(eq? v 'get-winner)
                ((k (state-winner? accum)) accum)]
               [else
                (define st v)
                (let loop ([st st] [accum accum])
                  (cond
                    [(stream-empty? st) accum]
...
(module+ test
  ...
  (define b2-won (json->board
                  (bytes-append #"{\"turn\":19,\"players\":[[[3,5],[2,5]],[[4,4],[3,4]]],"
                                #"\"spaces\":[[0,0,0,0,2],[1,1,2,0,0],[1,0,0,3,0],[0,0,3,0,0],[0,0,0,1,4]]}")))
  ...
  (check (and (member b2-won (all-next-moves b)) #t) #t)
  (check (and (member b2 (all-next-moves b)) #t) #f)
